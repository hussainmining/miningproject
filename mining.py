...
class Net(nn.Module):
     ...
     def __init__(self):
        ...
        self.fc1 = nn.Linear(31984, 4096) #in features, out features
     ...
     def forward(self, x):
        ...
        x=F.max_pool1d(F.relu(self.conv1(x)),4,2)        
...
def loader_function(path):
    ...
    return dataset,torch_data.DataLoader(dataset,batch_size=64,shuffle=True,num_workers=16)
#################################################################################################
...
def train(net,device):
   ...
   #weights saving
    ...
    with open('Simpleweight.txt', 'w') as f:
        for item in weights:
            f.write("%s\n" % item)
    pass
